...
import { createMockUser, useAuthStore } from '@/store/authStore';
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="auth-form" noValidate>
        ...
        <button type="submit" className="auth-cta" id="phone-submit-btn">
          Send Code
        </button>
      </form>
...
function OtpStep({ phone, onVerify, onBack }: OtpStepProps) {
  ...
  function handleComplete(val: string) {
    // Mock: accept "000000" as valid OTP
    if (val === '000000') {
      setVerified(true);
      setError('');
    } else {
      setError('Incorrect code. Hint: 000000');
    }
  }
  ...
  const maskedPhone = `+91 XXXXX ${phone.slice(-5)}`;
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
    >
      <button onClick={onBack} className="auth-back" id="otp-back-btn" aria-label="Go back">
        ← Back
      </button>
      <p className="auth-overline">Verification</p>
      <h1 className="auth-headline">Check your<br />messages.</h1>
      <p className="auth-subtext">
        Code sent to {maskedPhone}.<br />
        <span style={{ opacity: 0.4 }}>(Hint: use 000000)</span>
      </p>
      ...
      <form onSubmit={handleSubmit} className="auth-form" noValidate>
        ...
        {verified && (
          ...
          >
            <label htmlFor="name-input" className="auth-label">Your Name</label>
            <input
              id="name-input"
              type="text"
              value={name}
        ...
        <button type="submit" className="auth-cta" id="otp-verify-btn" disabled={!verified}>
          {verified ? 'Enter StreetPlayR' : 'Verifying…'}
        </button>
      </form>
...
function GoogleAuthButton() {
  ...
  // In production: trigger Google Identity Services popup/redirect.
  // For static mock: simulate immediate success with a Google user.
...
function LoginInner() {
  ...
  function handleVerify(name: string) {
    const mockUser = createMockUser(name, phone, 'phone');
    login(mockUser);
    const redirect = searchParams.get('redirect') ?? '/profile';
