...
function MaterialTile({ tile, index }: { tile: (typeof TILES)[number]; index: number }) {
  return (
    <FadeIn delay={0.1 * index}>
      <div className="group relative bg-[var(--panel)] border border-[var(--fg-06)] rounded-xl overflow-hidden">
        <div className="grid grid-rows-[3fr_1fr] aspect-[3/4]">
          <div className="relative overflow-hidden">
            ...
            <div className="absolute top-4 left-4 z-10">
              <span className="font-mono text-[7px] uppercase tracking-[0.3em] text-white/60">
                {tile.ref}
              </span>
            </div>
          </div>
          <div className="bg-[var(--panel)] px-5 py-4 flex flex-col justify-center">
            <span className="font-mono text-[6px] uppercase tracking-[0.3em] text-[#ddb7ff]/40 block leading-none mb-2.5">
              Material Spec
            </span>
            <h3 className="font-display text-[13px] sm:text-[14px] uppercase text-[var(--fg)] leading-[1.15] mb-1.5">
              {tile.name}
            </h3>
            <p className="font-body text-[9px] leading-[1.5] text-[var(--fg-35)] tracking-wide line-clamp-2 mb-2">
              {tile.description}
            </p>
            <div className="pt-2 border-t border-[var(--fg-06)]">
              <div className="flex flex-wrap gap-x-2.5 gap-y-0.5">
                {tile.specs.map((s) => (
                  <span key={s} className="font-mono text-[7px] tracking-[0.02em] text-[var(--fg-25)] flex items-center gap-1">
      ...
      <div className="mb-16">
        <FadeIn>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-[var(--fg-30)]">Material Specifications</span>
          </div>
        </FadeIn>
        <FadeIn delay={0.15}>
          <h2 className="font-display text-[clamp(40px,6vw,96px)] leading-[0.88] tracking-[0.01em] uppercase text-[var(--fg)]">
