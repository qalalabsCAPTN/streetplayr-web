...
import PremiumCoverflowGallery from "@/components/ui/PremiumCoverflowGallery";
...
export default function ProductGallery({ images, title, heroImage }: ProductGalleryProps) {
  ...
  const allImages = images.length > 0 ? images : [heroImage];

  return (
    <div className="w-full lg:h-full">
      {/* Mobile view CoverFlow Gallery (only visible below lg) */}
      <div className="block lg:hidden w-full py-4">
        <PremiumCoverflowGallery images={allImages} title={title} />
      </div>

      {/* Desktop view (100% untouched visual representation — only shown on lg and above) */}
      <div className="hidden lg:flex w-full flex-col lg:flex-row lg:h-full">
